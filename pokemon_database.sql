...
SELECT * FROM pokemon order by primary_type;

-- What are the average defence for all pokemon?

SELECT AVG(defence) as defenceAVG FROM pokemon;

-- What are the average hp for (primary) grass types?

SELECT AVG(hp) as AVGhp FROM pokemon WHERE 'grass';

-- How many pokemon exists pr. primary type?

SELECT COUNT(primary_type) from pokemon;
